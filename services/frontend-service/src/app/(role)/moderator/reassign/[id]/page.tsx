...
const ReassignTicketDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAssignees(usersData.results || []);
        setSelectedDepartment(ticketData.department || '');
      } catch (error: any) {
  ...
  if (!ticket) {
    return (
      <div className="p-6">
        ...
        <Button onClick={() => router.push('/moderator/reassign')}>
          Go Back
        </Button>
      </div>
    );
  ...
  const handleReassign = async () => {
    ...
    try {
      await ticketService.reassignTicket(ticketId, selectedAssignee, reassignReason);
          ...
          <CardContent className="p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="font-semibold text-gray-900">{ticket.ticketId || ticket.id}</p>
        </div>
